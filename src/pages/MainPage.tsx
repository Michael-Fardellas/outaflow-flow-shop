...
const MainPage = () => {
  ...
  const [particleTriggers, setParticleTriggers] = useState<{
    [key: string]: number;
  }>({});
  ...
  return <div className="bg-background text-foreground min-h-screen relative overflow-x-hidden">
      ...
      <div className="fixed top-0 left-0 w-full h-1 z-[60] pointer-events-none bg-black/20">
        ...
        background: currentSection === "butterfly" ? "hsla(185, 95%, 65%, 1)" : currentSection === "helmet" ? "hsla(239, 95%, 75%, 1)" : currentSection === "fire" ? "hsla(217, 98%, 68%, 1)" : "hsl(0, 0%, 100%)",
        boxShadow: currentSection === "butterfly" ? "0 0 30px hsla(185, 95%, 65%, 0.8), 0 0 60px hsla(185, 95%, 65%, 0.4)" : currentSection === "helmet" ? "0 0 30px hsla(239, 95%, 75%, 0.8), 0 0 60px hsla(239, 95%, 75%, 0.4)" : currentSection === "fire" ? "0 0 30px hsla(217, 98%, 68%, 0.8), 0 0 60px hsla(217, 98%, 68%, 0.4)" : "0 0 10px hsla(0, 0%, 100%, 0.3)",
      ...
      <section className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-4">
        ...
        <div className="absolute bottom-12 md:bottom-16 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 md:gap-4 opacity-0 animate-[hero-fade-in_0.8s_ease-out_forwards] pointer-events-none" style={{
        animationDelay: "2.2s"
      }}>
      ...
      <main className="space-y-0" style={{
      scrollBehavior: 'auto'
    }}>
        ...
        return <section key={product.node.id} id={sectionId} ref={el => sectionRefs.current[sectionId] = el} className={`${sceneClass} min-h-screen flex items-center py-16 md:py-20 px-8 relative overflow-visible transition-opacity duration-700 ${isVisible ? "opacity-100" : "opacity-0"}`}>
              {/* SCATTERED DECORATIONS WITH RADIAL HUES */}
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-0" style={{
            transform: "scale(1)",
            opacity: 0.95,
            transition: "all 1.2s cubic-bezier(0.4, 0, 0.2, 1)"
                ...
                {theme === 'butterfly' && <>
                    ...
                    <div className="absolute -top-[60vh] -bottom-[60vh] left-0 right-0 overflow-visible pointer-events-none">
                      <div className="absolute inset-0 blur-[200px]" style={{
                  ...
                  delay: 0.5,
                  duration: 17
                }].map((pos, i) => <svg key={i} className="absolute animate-[float_ease-in-out_infinite]" style={{
                  top: pos.top,
                  left: pos.left,
                  right: pos.right,
                  width: `${pos.size}px`,
                ...
                {theme === 'helmet' && <>
                  ...
                  delay: 0.5,
                  duration: 17
                }].map((pos, i) => <Flower2 key={i} className="absolute animate-[float_ease-in-out_infinite]" style={{
                  top: pos.top,
                  left: pos.left,
                  right: pos.right,
                  width: `${pos.size}px`,
                ...
                {theme === 'fire' && <>
                  ...
                  delay: 2.5,
                  duration: 19
                }].map((pos, i) => <Heart key={`heart-${i}`} className="absolute animate-[float_ease-in-out_infinite]" style={{
                  top: pos.top,
                  left: pos.left,
                  right: pos.right,
                  width: `${pos.size}px`,
                ...
                }} fill="hsla(217, 98%, 68%, 0.5)" strokeWidth={2} />)}
                    </div>
                  </>}
 
                {/* Fallback: neutral subtle halo with smooth transitions if theme doesn't match */}
                {theme === 'default' && <div className="absolute inset-0 overflow-visible" style={{
              margin: '-30vh 0'
            }}>
                    <div className="absolute inset-0 blur-[200px]" style={{
              ...
              <div className={`max-w-6xl mx-auto w-full grid gap-12 md:gap-16 md:grid-cols-2 items-center relative z-10 ${isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"} transition-all duration-900 delay-150`}>
                {/* Text column - Staggered fade-in - Always second on mobile */}
                <div className={`relative z-10 space-y-6 order-2 ${isEven ? "md:order-2" : "md:order-1"} ${handle.includes("fire") || handle.includes("love") ? "tracking-wider" : ""}`}>
                  {/* Product title - larger but thinner */}
                  <h2 className={`text-4xl md:text-5xl font-bold tracking-[0.08em] uppercase mb-3 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"} ${handle.includes("fire") || handle.includes("love") ? "text-[hsl(217,98%,68%)]" : ""}`} style={{
                transitionDelay: "400ms"
                  ...
                  <p className={`text-sm font-light tracking-[0.25em] uppercase opacity-60 mb-6 transition-opacity duration-1000 ${isVisible ? "opacity-60" : "opacity-0"} ${handle.includes("fire") || handle.includes("love") ? "text-[hsl(217,98%,68%)]" : ""}`} style={{
                transitionDelay: "600ms"
                  ...
                  <div className={`max-w-md space-y-2 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"} ${handle.includes("fire") || handle.includes("love") ? "text-[hsl(217,98%,68%)]" : ""}`} style={{
                transitionDelay: "800ms"
                  ...
                  {/* Price */}
                  <p className={`text-3xl font-extralight tracking-wider mb-8 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"} ${handle.includes("fire") || handle.includes("love") ? "text-[hsl(217,98%,68%)]" : ""}`} style={{
                transitionDelay: "1000ms"
                  ...
                  {/* Size selector */}
                  <div className={`space-y-4 mb-8 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`} style={{
                transitionDelay: "1200ms"
              }}>
                    <p className="text-[10px] uppercase tracking-[0.3em] font-light opacity-50">Select Size</p>
                    <div className="flex flex-wrap gap-3">
                      {product.node.variants.edges.map((variant, idx) => <button key={variant.node.id} onClick={() => {
                    playClick();
                    ...
                    setSelectedSizes(prev => ({
                      ...
                      [product.node.id]: idx
                            ...
                            ${selectedSizes[product.node.id] === idx ? "bg-foreground/5 text-foreground border-foreground/40" : "bg-transparent text-foreground/60 border-foreground/20 hover:border-foreground/40 hover:text-foreground/80"}
                          `}>
                          {variant.node.title}
                        </button>)}
                    </div>
                  ...
                  <Button onClick={e => handleAddToCart(product, e)} className={`w-full mb-6 transition-all duration-500 h-12 text-xs tracking-[0.25em] font-light bg-transparent border border-foreground/30 text-foreground hover:bg-foreground/5 hover:border-foreground/50 ${isVisible ? "opacity-100" : "opacity-0"}`} style={{
                transitionDelay: "1400ms"
                  ...
                  <details className={`mt-6 border-t border-foreground/10 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`} style={{
                ...
                <div className={`relative z-10 order-1 ${isEven ? "md:order-1" : "md:order-2"}`}>
                  ...
                  playClick();
                  if ('vibrate' in navigator) navigator.vibrate(30);
                  setSelectedImageView(prev => ({
                    ...prev,
                    [product.node.id]: "front"
                  }));
                }} className={`pb-2 relative transition-all duration-300
                        ${currentView === "front" ? "text-foreground opacity-100" : "text-foreground/40 hover:text-foreground/70"}
                      `}>
                      Front
                      <div className={`absolute bottom-0 left-0 right-0 h-[1px] bg-foreground transition-all duration-300 ${currentView === "front" ? "opacity-100" : "opacity-0"}`} />
                    </button>
                    <button onClick={() => {
                  playClick();
                  if ('vibrate' in navigator) navigator.vibrate(30);
                  setSelectedImageView(prev => ({
                    ...prev,
                    [product.node.id]: "back"
                  }));
                }} className={`pb-2 relative transition-all duration-300
                        ${currentView === "back" ? "text-foreground opacity-100" : "text-foreground/40 hover:text-foreground/70"}
                      `}>
                      Back
                      <div className={`absolute bottom-0 left-0 right-0 h-[1px] bg-foreground transition-all duration-300 ${currentView === "back" ? "opacity-100" : "opacity-0"}`} />
                    </button>
                  </div>

                  {/* Product image container */}
                  <div className="relative">
                    <Link to={`/product/${product.node.handle}`} className="block relative group">
                      <div className="relative">
                        <img key={currentView} src={product.node.images.edges[imageIndex]?.node.url} alt={`${product.node.title} ${currentView}`} className="w-full h-auto transition-opacity duration-[160ms] ease-out relative z-10" style={{
                      willChange: "opacity"
                    }} />
        ...
        <section className="min-h-[60vh] flex items-center justify-center py-[20vh] px-8 border-t border-foreground/5 relative">
          <div className="max-w-lg mx-auto text-center space-y-8 relative z-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Get notified on next drops" className="h-12 bg-transparent border border-foreground/20 text-center text-xs tracking-[0.2em] font-light focus:border-foreground/40 transition-all duration-500" />
          ...
          <a href="https://instagram.com/mixalis_fardellas" target="_blank" rel="noopener noreferrer" onClick={() => {
          playClick();
          if ("vibrate" in navigator) navigator.vibrate(30);
      ...
      {sizeChartModalOpen && <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center px-8 backdrop-blur-sm" onClick={() => setSizeChartModalOpen(null)}>
          <div className="bg-background border border-foreground/10 max-w-4xl w-full max-h-[85vh] overflow-auto p-10" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8 pb-6 border-b border-foreground/10">
              ...
              <button onClick={() => setSizeChartModalOpen(null)} className="text-2xl leading-none opacity-50 hover:opacity-100 transition-opacity w-8 h-8 flex items-center justify-center" aria-label="Close size chart">
                ×
              </button>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm font-light border-collapse">
                ...
                <tbody>
                  {sizeCharts[sizeChartModalOpen].sizes.map((row, idx) => <tr key={idx} className="border-b border-foreground/5">
      ...
      return <ParticleEffect key={product.node.id} type={particleType} color={particleColor} trigger={particleTriggers[product.node.id] || 0} originX={position?.x} originY={position?.y} />;
    })}
    </div>;
};
